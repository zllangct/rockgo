...
type CommandInterpreter struct {
	commands map[string]RockInterface.ICommand
}

func NewCommandInterpreter() *CommandInterpreter{
	interpreter := &CommandInterpreter{make(map[string]RockInterface.ICommand)}
	return interpreter
}

func (this *CommandInterpreter)AddCommand(cmd RockInterface.ICommand){
