...
const (
	...
	RETRY_INTERVAL = 5   //重连间隔60s
)

type TcpClient struct {
	conn          *net.TCPConn
	addr          *net.TCPAddr
	protoc        RockInterface.IClientProtocol
	PropertyBag   map[string]interface{}
	reconnCB      func(RockInterface.Iclient)
	maxRetry      int
	retryInterval int
...
func NewReConnTcpClient(ip string, port int, protoc RockInterface.IClientProtocol, maxRetry int,
	retryInterval int, reconnCB func(RockInterface.Iclient)) *TcpClient {
...
func NewTcpClient(ip string, port int, protoc RockInterface.IClientProtocol) *TcpClient {
	addr := &net.TCPAddr{
		IP:   net.ParseIP(ip),
		Port: port,
		Zone: "",
	...
	if err == nil {
		...
		client.SetProperty("addr",addr.String())
