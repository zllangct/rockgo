...
type PoolConfig struct {
	...
	//关闭链接的方法
	Close func(writer RockInterface.IWriter) error
	//链接最大空闲时间，超过该事件则将失效
...
func NewChannelPool(poolConfig *PoolConfig) (Pool, error) {
	...
	for i := 0; i < poolConfig.InitialCap; i++ {
		...
		if err != nil {
			c.Release()
			return nil, fmt.Errorf("factory is not able to fill the ConnPool: %s", err)
		}
...
//Get 从pool中取一个连接
func (c *channelPool) Get() (RockInterface.IWriter, error) {
	conns := c.getConns()
	if conns == nil {
...
//Put 将连接放回pool中
func (c *channelPool) Put(conn RockInterface.IWriter) error {
	if conn == nil {
...
//Close 关闭单条连接
func (c *channelPool) Close(conn RockInterface.IWriter) error {
	if conn == nil {
		return errors.New("connection is nil. rejecting")
	}
	return c.close(conn)
